...
#
# Copyright (C) 2023  Alexander Bazarov <alexander[at]bazarov.dev>
#
...
class aip31068_spi9bit:
    ...
    def send(self, data, minclock=0):
        for elem in data:
            self.spi.spi_send_arb1(elem, minclock)
    def flush(self):
        ...
        for new_data, old_data, fb_cmnd in self.all_framebuffers:
            ...
            for pos, count in diffs:
                ...
                self.send([CMND.WRITE_RAM | byte for byte in new_data[pos:pos+count]])
            old_data[:] = new_data
    def init(self):
